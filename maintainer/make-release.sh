...
if [[ ${prerelease} == "" ]] && [[ -f CHANGELOG.md ]]; then
    sed -i \
        -e "s:\(## \[Unreleased\]\):\1\n\n## [${version}] - $(date +%d/%m/%Y):" \
        -e "s:\[Unreleased\]\(\: https\://github.com/tanjona/mchep/compare/v\)\(.*\)...HEAD:[Unreleased]\1${version}...HEAD\n[${version}]\1\2...v${version}:" \
        CHANGELOG.md
...
# update mchep version in its dependents
for crate in mchep_capi mchep_pyapi; do
    if [[ -f "${crate}/Cargo.toml" ]]; then
        sed -i \
            -e "s:mchep = { path = \"\.\./mchep\" }:mchep = { path = \"../mchep\", version = \"${version}\" }:" \
            -e "s:mchep = { path = \"\.\./mchep\", version = \"[^\"]*\" }:mchep = { path = \"../mchep\", version = \"${version}\" }:" \
            "${crate}/Cargo.toml"
